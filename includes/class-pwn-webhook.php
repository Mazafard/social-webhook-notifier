...
class PWN_Webhook {
    ...
    private function init_hooks() {
        ...
        PWN_Logger::log('Webhook hooks registered');
    }
    ...
    public function notify_webhook($post_id, $post) {
        PWN_Logger::log('Webhook notification triggered for post ID: ' . $post_id);
        
        // Check if webhook is enabled
        if (!get_option('pwn_enabled', '1')) {
            PWN_Logger::log('Webhook disabled, skipping notification');
            return;
        }
        
        $webhook_url = get_option('pwn_webhook_url');
        if (!$webhook_url) {
            PWN_Logger::warning('No webhook URL configured');
            return;
        }
        ...
        switch ($settings['auth_type']) {
            case 'basic':
                if ($settings['basic_username'] && $settings['basic_password']) {
                    ...
                    PWN_Logger::log('Added Basic authentication header');
                }
                break;
            ...
            case 'header':
                if ($settings['header_name'] && $settings['header_value']) {
                    ...
                    PWN_Logger::log('Added custom header: ' . $settings['header_name']);
                }
                break;
            ...
            case 'jwt':
                if ($settings['jwt_secret'] && $settings['jwt_algorithm']) {
                    ...
                    if ($jwt_token) {
                        ...
                        PWN_Logger::log('Added JWT authentication header');
                    } else {
                        PWN_Logger::warning('Failed to generate JWT token');
                    }
                }
                break;
    ...
    private function send_webhook($webhook_url, $method, $headers, $body) {
        ...
        $test_mode = get_option('pwn_test_mode', '0');
        $log_prefix = $test_mode ? '[TEST MODE] ' : '';
        
        PWN_Logger::log($log_prefix . 'Sending webhook to: ' . $webhook_url . ' with method: ' . $method);
        
        // Skip actual request in test mode
        if ($test_mode) {
            PWN_Logger::log('[TEST MODE] Request simulation - would send: ' . $body);
            return;
        }
        ...
        if (is_wp_error($response)) {
            ...
            PWN_Logger::error('Webhook request failed: ' . $response->get_error_message());
        } else {
            $response_code = wp_remote_retrieve_response_code($response);
            PWN_Logger::log('Webhook sent successfully. Response code: ' . $response_code);
            
            // Log response body for debugging if needed
            if (defined('WP_DEBUG') && WP_DEBUG) {
                $response_body = wp_remote_retrieve_body($response);
                if ($response_body) {
                    PWN_Logger::log('Response body: ' . substr($response_body, 0, 500));
                }
            }
        }
    }
